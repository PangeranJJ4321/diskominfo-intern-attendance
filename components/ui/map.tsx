...
/**
 * A wrapper component around Leaflet's GeoJSON layer.
 * This component is loaded dynamically to avoid server-side rendering errors.
 *
 * @param props - The properties for the GeoJSON layer.
 * @returns The rendered JSX element.
 */
function MapGeoJSON(props: GeoJSONProps) {
  return <LeafletGeoJSON {...props} />;
}
...
function MapDrawControl({
  ...
  const initialLayersSignature = JSON.stringify(initialLayers ?? []);
  ...
  useEffect(() => {
    if (!L || !LeafletDraw || !map || !featureGroupRef.current) return;

    const featureGroup = featureGroupRef.current;
    featureGroup.clearLayers();

    for (const initialLayer of initialLayers ?? []) {
      L.geoJSON(initialLayer, {
        style: {
  ...
  }, [L, LeafletDraw, map, initialLayersSignature]);
