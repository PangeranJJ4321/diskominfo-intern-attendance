import { AgencyAccess } from "@/interfaces/models";
 ...
 */
export async function getAgencyAccesses(limit = 1000): Promise<AgencyAccess[]> {
...
/**
 * Updates an existing agency access.
 *
 * @param {string} id - The access ID.
 * @param {Partial<Pick<AgencyAccess, "agencyId" | "userId">>} data - The data to update.
 * @returns {Promise<AgencyAccess>} The updated agency access.
 */
export async function updateAgencyAccess(
  ...
  if (!res.ok) await handleError(res, "Gagal memperbarui akses instansi");
  return res.json();
}
 ...
 */
export async function deleteAgencyAccess(id: string): Promise<void> {
  const res = await fetch(`/api/agency-accesses/${id}`, {
    method: "DELETE",
  });
